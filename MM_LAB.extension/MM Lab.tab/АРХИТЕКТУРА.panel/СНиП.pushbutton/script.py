# -*- coding: utf-8 -*-
__title__ = 'Проверка площадей (Старый ФОП)'
__author__ = 'Akhmedyanova Eleonora'
__doc__= 'Проверяет площади помещений на соответствие требованиям СНиП (Genpro_SP)'
...
# КОЭФФИЦИЕНТ ПЕРЕВОДА КВАДРАТНЫХ ФУТОВ В КВАДРАТНЫЕ МЕТРЫ
FT2_TO_M2 = 0.09290304

# Сетевой путь к старому корпоративному ФОП
SHARED_PARAM_PATH = r"\\srv-dfs\BIM\01_Ресурсы плагинов\2_Shared_parameters\Genpro_SP.txt"

# Корни слов для распознавания кухонь, кухонных зон, ниш, столовых
KITCHEN_KEYWORDS = ["кух", "зон", "ниш", "столов", "гостин"]

# ===================================================================
# GUID СТАРЫХ ПАРАМЕТРОВ (Genpro_SP.txt)
# ===================================================================
...
GUID_MAP = {
    ...
    "AREA":       "2e5be1cd-51ae-494b-8e14-54e7aa1d7ae9",  # GP_Площадь
    "PURPOSE":    "45f41e98-8e57-4112-a573-a1f2b2697dac",  # GP_Назначение
    "APT_NUM":    "2958ad76-9984-4cd5-9791-648954471f55",  # GP_Номер квартиры на этаже
    "ROOM_COUNT": "d3d671d4-f902-4308-a6ec-54f8f6ae296a",  # GP_Количество комнат
    "ROOM_TYPE":  "858d6e67-d43a-48da-a329-76678becb5c2"   # GP_Тип помещения_Номер
}

# ===================================================================
# 1. ПРИВЯЗКА ПАРАМЕТРОВ ИЗ СТАРОГО ФОП
# ===================================================================
...
def get_param_val(element, key_name):
    """Считывает значение параметра по его GUID из старого ФОП."""
...
def get_room_area(element):
    """
    Считывает площадь помещения.
    Переводит квадратные футы Revit API в квадратные метры.
    """
    guid_str = GUID_MAP.get("AREA")
    param = element.get_Parameter(System.Guid(guid_str)) if guid_str else None
    
    if param and param.HasValue:
        if param.StorageType == StorageType.Double:
            # Чтение внутреннего числа из Revit API и перевод футов² в м²
            return param.AsDouble() * FT2_TO_M2
        elif param.StorageType == StorageType.Integer:
            return float(param.AsInteger()) * FT2_TO_M2
    ...
    # Фолбэк на родную системную площадь Revit (если пользовательский параметр пуст)
...
def set_param_val(element, key_name, value):
    """Записывает значение в параметр старого ФОП."""
    guid_str = GUID_MAP.get(key_name)
    if guid_str:
...
if not filtered_rooms:
    forms.alert("Нет помещений с 'квартир' в параметре GP_Назначение!", exitscript=True)
    sys.exit()
