# -*- coding: utf-8 -*-
__title__ = "Замена CAD-геометрии"
__author__ = "Артём Брежнев"
__doc__ = "Замена DWG-импортов в семействе на лёгкие нативные кубики с сохранением коннекторов"
...
if not doc.IsFamilyDocument:
    TaskDialog.Show("Замена CAD-геометрии", "Открой .rfa файл в редакторе семейств")
else:
    log = []
    ...
    for ce in FilteredElementCollector(doc).OfClass(ConnectorElement).ToElements():
        try:
            ...
            except: d["domain"]  = None
            try:    d["profile"] = ce.Shape
            except: d["profile"] = ConnectorProfileType.Round
            ...
            try:
                if d["domain"] == Domain.DomainHvac:
                    p = ce.get_Parameter(BuiltInParameter.RBS_DUCT_CONNECTOR_SYSTEM_CLASSIFICATION_PARAM)
                    if p is None or not p.HasValue:
                        p = ce.get_Parameter(BuiltInParameter.RBS_DUCT_CONNECTOR_SYSTEM_TYPE_PARAM)
                    if p and p.HasValue: d["sys_type_int"] = p.AsInteger()
                elif d["domain"] == Domain.DomainPiping:
                    p = ce.get_Parameter(BuiltInParameter.RBS_PIPE_CONNECTOR_SYSTEM_CLASSIFICATION_PARAM)
                    if p is None or not p.HasValue:
                        p = ce.get_Parameter(BuiltInParameter.RBS_PIPE_CONNECTOR_SYSTEM_TYPE_PARAM)
                    if p and p.HasValue: d["sys_type_int"] = p.AsInteger()
            except: pass

            d["flow"] = None
            try:
                p = ce.get_Parameter(BuiltInParameter.RBS_DUCT_CONNECTOR_FLOW_DIRECTION_PARAM)
                if p is None or not p.HasValue:
                    p = ce.get_Parameter(BuiltInParameter.RBS_PIPE_CONNECTOR_FLOW_DIRECTION_PARAM)
    ...
    if not imports:
        TaskDialog.Show("Замена CAD-геометрии",
        ...
        try:
                ...
                for i, s in enumerate(saved):
                    ...
                    try:
                        ...
                        if s["sys_type_int"] is not None:
                            try:
                                if domain == Domain.DomainHvac:
                                    p = ce_new.get_Parameter(BuiltInParameter.RBS_DUCT_CONNECTOR_SYSTEM_CLASSIFICATION_PARAM)
                                    if p is None or p.IsReadOnly:
                                        p = ce_new.get_Parameter(BuiltInParameter.RBS_DUCT_CONNECTOR_SYSTEM_TYPE_PARAM)
                                    if p and not p.IsReadOnly:
                                        p.Set(s["sys_type_int"])
                                elif domain == Domain.DomainPiping:
                                    p = ce_new.get_Parameter(BuiltInParameter.RBS_PIPE_CONNECTOR_SYSTEM_CLASSIFICATION_PARAM)
                                    if p is None or p.IsReadOnly:
                                        p = ce_new.get_Parameter(BuiltInParameter.RBS_PIPE_CONNECTOR_SYSTEM_TYPE_PARAM)
                                    if p and not p.IsReadOnly:
                                        p.Set(s["sys_type_int"])
                            except Exception as e:
                        ...
                        try:
                            if s["radius"]:
                                p = ce_new.get_Parameter(BuiltInParameter.CONNECTOR_RADIUS)
                                if p and not p.IsReadOnly: p.Set(s["radius"])
                        except: pass
                        try:
                            if s["width"]:
                                p = ce_new.get_Parameter(BuiltInParameter.CONNECTOR_WIDTH)
                                if p and not p.IsReadOnly: p.Set(s["width"])
                            if s["height"]:
                                p = ce_new.get_Parameter(BuiltInParameter.CONNECTOR_HEIGHT)
                                if p and not p.IsReadOnly: p.Set(s["height"])
                        except: pass

                        if s["flow"] is not None:
                            try:
                                p = ce_new.get_Parameter(BuiltInParameter.RBS_DUCT_CONNECTOR_FLOW_DIRECTION_PARAM)
                                if p is None or p.IsReadOnly:
                                    p = ce_new.get_Parameter(BuiltInParameter.RBS_PIPE_CONNECTOR_FLOW_DIRECTION_PARAM)
                                if p and not p.IsReadOnly:
                                    p.Set(s["flow"])
                            except: pass
            ...
            TaskDialog.Show("Замена CAD-геометрии: ошибка", "Произошла ошибка: " + str(e))
        else:
            summary = (
                "Коннекторов было:    {}\n"
                "Коннекторов создано: {}\n"
            ...
            TaskDialog.Show("Замена CAD-геометрии: готово", summary)