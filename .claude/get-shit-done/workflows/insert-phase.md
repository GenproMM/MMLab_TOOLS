...
```bash
INIT=$(node "G:/Мой диск/02_DEV/01_Python/GENPRO LAB/MMLab_TOOLS/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "G:/Мой диск/02_DEV/01_Python/GENPRO LAB/MMLab_TOOLS/.claude/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
