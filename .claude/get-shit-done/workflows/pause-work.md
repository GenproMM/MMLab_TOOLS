...
```bash
timestamp=$(node "G:/Мой диск/02_DEV/01_Python/GENPRO LAB/MMLab_TOOLS/.claude/get-shit-done/bin/gsd-tools.cjs" current-timestamp full --raw)
```
...
<step name="commit">
```bash
node "G:/Мой диск/02_DEV/01_Python/GENPRO LAB/MMLab_TOOLS/.claude/get-shit-done/bin/gsd-tools.cjs" commit "wip: [context-name] paused at [X]/[Y]" --files [handoff-path] .planning/HANDOFF.json
```
</step>
