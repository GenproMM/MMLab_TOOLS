...
```bash
PHASE_INFO=$(node "G:/Мой диск/02_DEV/01_Python/GENPRO LAB/MMLab_TOOLS/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "G:/Мой диск/02_DEV/01_Python/GENPRO LAB/MMLab_TOOLS/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "G:/Мой диск/02_DEV/01_Python/GENPRO LAB/MMLab_TOOLS/.claude/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
