...
- **First-party lib** — `MM_LAB.extension/lib`: pyRevit сам добавляет этот каталог в
  `sys.path` всем командам расширения. Правки в `lib` требуют **полного перезапуска
  Revit**: pyRevit Reload перечитывает `script.py` кнопок, но уже импортированные
  lib-модули остаются в `sys.modules` движка и продолжают исполнять старый код
  (см. §Перезагрузка после правок).
...
| Что изменено | Чего достаточно |
| ----------------------------------- | ---------------------------------- |
| `script.py` кнопки | pyRevit Reload |
| состав кнопок, `bundle.yaml`, layout | pyRevit Reload (расширение парсится на старте Revit) |
| любой модуль в `MM_LAB.extension/lib` | **полный перезапуск Revit** |

Reload НЕ перезагружает lib: модуль, однажды импортированный в этой сессии Revit,
остаётся в `sys.modules` движка, и кнопка продолжает исполнять старый код молча —
без ошибок и предупреждений. Симптом обманчив: правка «не работает», хотя код верен.

Чтобы отличать этот случай от настоящей ошибки, в lib-модуле держат метку версии
(например `LIB_VERSION` в `ios_common_helpers.py`) и печатают её в отладочном
выводе: отсутствие или устаревшее значение метки однозначно указывает на
несброшенный `sys.modules`, а не на дефект логики. Если перезапуск Revit не помог —
сбросить кэш pyRevit (Settings → Reset/Purge caches) и перезапустить снова.
