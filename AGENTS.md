...
**Baseline** (`tools/convention_baseline.json`) — замороженные нарушения legacy-кнопок:
старые кнопки не «краснят» общий прогон, но и не растят долг — новые нарушения baseline
не покрывает. При адаптации кнопки (`/mm-adopt-script`) её запись из baseline удаляется,
дальше кнопка держит `--strict`.
